...
class CustomIOMToolkit:
    ...
    def _update_policy_interactive(self, rule: Dict):
        ...
        print(f"2. {Colors.RED}⚠️  Severity{Colors.ENDC}")
        print(f"3. {Colors.YELLOW}✏️  Edit Policy Logic (Rego code){Colors.ENDC}")
        ...
        if choice == "1":
            ...
            while True:
                ...
                print(f"2. {Colors.BLUE}✏️  Edit Logic Again{Colors.ENDC}")
    ...
    def _delete_policy_interactive(self, rule: Dict):
        ...
        print(f"\n{Colors.RED}⚠️  You are about to delete this policy:{Colors.ENDC}")
        print(format_rule_card(rule))
    ...
    def _display_test_results_fixed(self, test_results: List[Dict], resource_type: str):
        ...
        for i, result in enumerate(test_results, 1):
            ...
            if evaluation_result == "pass":
                print(f"  {Colors.GREEN}✅ Asset {i}: {display_id} → PASS{Colors.ENDC}")
                pass_count += 1
            elif evaluation_result == "fail":
                print(f"  {Colors.RED}❌ Asset {i}: {display_id} → FAIL{Colors.ENDC}")
                fail_count += 1
                ...
                print(f"  {Colors.YELLOW}⚠️  Asset {i}: {display_id} → ERROR{Colors.ENDC}")
        ...
        if total_assets > 0:
            ...
            if error_count > 0:
                print(f"  {Colors.YELLOW}⚠️  Errors: {error_count}/{total_assets} ({error_count/total_assets*100:.1f}%){Colors.ENDC}")
    ...
    def _display_test_results(self, results: Dict, asset_ids: List[str], resource_type: str):
        ...
        for i, result in enumerate(resources, 1):
            ...
            if evaluation_result == "pass":
                ...
                print(f"  {Colors.YELLOW}⚠️  Asset {i}: {display_id} → ERROR/UNKNOWN{Colors.ENDC}")
                error_count += 1
...
"""
        ...
        while True:
            ...
            print(f"2. {Colors.BLUE}✏️  Edit Policy Logic{Colors.ENDC}")
    ...
    def main_menu(self):
        ...
        while True:
            ...
            print(f"{Colors.BOLD}{Colors.BLUE}                            🛠️  MAIN MENU                            {Colors.ENDC}")
            ...
            print(f"  {Colors.YELLOW}✏️  2.{Colors.ENDC} {Colors.BOLD}Update Existing Policy{Colors.ENDC} - Modify an existing custom policy")
            print(f"  {Colors.RED}🗑️  3.{Colors.ENDC} {Colors.BOLD}Delete Existing Policy{Colors.ENDC} - Remove a custom policy")
